...
public class InventoryService : ServiceBase, IService
{
	...
	// get total rap for all assets for collectibles
	public async Task<long> GetAllRAP(long userId, Models.Assets.Type? type)
	{
		var sql = new SqlBuilder();
		var t = sql.AddTemplate(
			"SELECT COALESCE(SUM(asset.recent_average_price), 0) as total FROM user_asset INNER JOIN asset ON asset.id = user_asset.asset_id /**where**/",
			new { user_id = userId });
		
		sql.Where("asset.is_limited AND user_asset.user_id = :user_id", new { user_id = userId });
		if (type != null)
		{
			sql.Where("asset.asset_type = :type", new { type = (int)type });
		}
	...
	public async Task<(IEnumerable<CollectibleItemEntry> items, long totalRap)> GetCollectibleInventoryGrouped(long userId, Models.Assets.Type? type,
		string sortOrder, int limit, int offset)
	{
		var acounts = await db.QueryAsync<(long assetId, int count)>(
			@"SELECT asset_id as assetId, COUNT(*) as count 
			FROM user_asset 
			INNER JOIN asset ON asset.id = user_asset.asset_id 
			WHERE asset.is_limited AND user_asset.user_id = @userId 
			AND (@type IS NULL OR asset.asset_type = @type)
			GROUP BY asset_id
			ORDER BY MAX(user_asset.id) " + (sortOrder == "desc" ? "DESC" : "ASC") + @"
			LIMIT @limit OFFSET @offset",
			new { userId, type = (int?)type, limit, offset });

		var assetIds = acounts.Select(x => x.assetId).ToList();
		if (assetIds.Count == 0)
			return (Enumerable.Empty<CollectibleItemEntry>(), 0);

		var items = await db.QueryAsync<CollectibleItemEntry>(
			@"SELECT 
				user_asset.id as userAssetId,
				user_asset.serial as serialNumber,
				asset.id as assetId, 
				asset.recent_average_price as recentAveragePrice, 
				asset.price_robux as originalPrice, 
				asset.serial_count as assetStock, 
				asset.asset_type as assetTypeId, 
				asset.name as name,
				1 as ownedCount
			FROM user_asset
			INNER JOIN asset ON asset.id = user_asset.asset_id
			WHERE asset.id = ANY(@assetIds) AND user_asset.user_id = @userId
			ORDER BY user_asset.id " + (sortOrder == "desc" ? "DESC" : "ASC"),
			new { userId, assetIds });
