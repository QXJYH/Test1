...
public class ThumbnailsService : ServiceBase, IService
{
	...
	public async Task<IEnumerable<ThumbnailEntry>> GetAssetThumbnails(IEnumerable<long> assetIds)
	{
		...
		if (ids.Count == 0) return Array.Empty<ThumbnailEntry>();

		var results = await db.QueryAsync<(long targetId, Type type, ModerationStatus moderationStatus, string thumbnailUrl, string contentUrl)>(
			@"SELECT 
				a.id as targetId, 
				a.asset_type as type, 
				a.moderation_status as moderationStatus,
				at.content_url as thumbnailUrl,
				(SELECT content_url FROM asset_version WHERE asset_id = a.id ORDER BY id DESC LIMIT 1) as contentUrl
			  FROM asset a
			  LEFT JOIN asset_thumbnail at ON at.asset_id = a.id
			  WHERE a.id = ANY(@ids)",
			new { ids });
		...
		return results.Select(c =>
		{
			...
			{
				if (c.type == Type.Image)
				{
					final = !string.IsNullOrEmpty(c.contentUrl) 
				...
				{
					final = "/images/thumbnails/" + c.thumbnailUrl + ".png";
				}
			}
			...
			{
				targetId = c.targetId,
				imageUrl = final != null ? Configuration.CdnBaseUrl + final : null,
				state = final == null ? ThumbnailState.Pending : ThumbnailState.Completed
			};
		});
	}
