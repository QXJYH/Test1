...
/* 	public async Task<bool> ValidateGameRBXL(Stream file, Models.Assets.Type assetType)
    ...
    public async Task<(Stream stream, string contentType)> GetAudioContent(long assetId, string contentUrl)
    {
        var legacyWavPath = Configuration.StorageDirectory + "/" + contentUrl + "_wav_cache_v1.wav";
        if (File.Exists(legacyWavPath))
        {
            var wavStream = new FileStream(legacyWavPath, FileMode.Open, FileAccess.Read, FileShare.Read, default, FileOptions.Asynchronous);
            return (wavStream, "audio/wav");
        }
        ...
        try
        {
            var analysis = await FFProbe.AnalyseAsync(assetContent);
            var format = analysis.Format.FormatName.ToLower();

            if (format.Contains("mp3"))
            {
                assetContent.Position = 0;
                return (assetContent, "audio/mpeg");
            }
            
            if (format.Contains("ogg"))
            {
                assetContent.Position = 0;
                return (assetContent, "audio/ogg");
            }

            throw new RobloxException(400, 0, "Invalid format. Only .mp3 and .ogg are supported.");
        }
        catch (Exception e) when (e is not RobloxException)
        {
            Writer.Info(LogGroup.AudioConversion, "Audio analysis failed for {0}: {1}", assetId, e.Message);
            throw new RobloxException(400, 0, "Failed to verify audio format.");
        }
    }
	...
	public async Task CreateGameIcon(long assetId, Stream? thumbnailToUse = null, CancellationToken? cancellationToken = null)
	{
		...
		else
		{
			...
			if (imageData.width < 128 || imageData.height < 128)
				throw new ArgumentException("Icon must be at least 128x128 pixels");
				
			if (imageData.width != imageData.height)
				throw new ArgumentException("Icon must have a square aspect ratio (width = height)");
	...
	public async Task<Stream?> GetGameIcon(long assetId)
	{
		var result = await db.QuerySingleOrDefaultAsync(
			"SELECT content_url FROM asset_icon WHERE asset_id = :id AND moderation_status = :status",
			new 
			{
				id = assetId,
		...
		if (result == null || result.content_url == null)
			return null;
			
		return await GetAssetContent(result.content_url);
	}
...
/* 			case Type.LeftArm:
		...
		DateTime? updatedAt = null, long? robloxAssetId = 0, bool disableRender = false, long? contentId = null, long? assetIdOverride = null, bool skipHashCheck = false)
	{
		...
		if (!disableRender && assetType != Models.Assets.Type.Image)
		{
    ...
    public async Task<ProductEntry> GetProductForAsset(long assetId)
    {
        var result = await db.QuerySingleOrDefaultAsync<ProductEntry>(
            "SELECT name, description, is_for_sale as isForSale, is_limited as isLimited, is_limited_unique as isLimitedUnique, price_robux as priceRobux, price_tix as priceTickets, serial_count as serialCount, offsale_at as offsaleAt, visible as isVisible, recent_average_price as recentAveragePrice FROM asset WHERE id = :id",
            new
            {
                id = assetId,
            });
	...
	public async Task UpdateAssetNameAndDesc(long assetId, string newName, string newDescription = null)
	{
		await UpdateAsync("asset", assetId, new
		{
			name = newName,
